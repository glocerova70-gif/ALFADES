    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
                    ...
                    <div className="p-4 bg-white border-t border-slate-100">
                        <div className="flex gap-2 relative">
                            <input
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-full px-4 py-3 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-glocerova-blue/50 focus:border-glocerova-blue transition-all"
                                disabled={isThinking}
                            />
                            <button
                                onClick={handleSend}
                                disabled={!inputValue.trim() || isThinking}
                                className="absolute right-1.5 top-1.5 p-1.5 bg-glocerova-blue text-white rounded-full hover:bg-blue-700 disabled:opacity-50 disabled:hover:bg-glocerova-blue transition-colors shadow-sm"
                            >
            ...
            <button
                ...
                className="pointer-events-auto bg-glocerova-gold text-glocerova-dark hover:bg-yellow-400 hover:scale-105 transition-all duration-300 shadow-xl rounded-full p-4 flex items-center gap-2 group"
            >
                {isOpen ? (
                    <X size={28} />
                ) : (
                    <>
                        ...
                        <span className="absolute -top-1 -right-1 flex h-3 w-3">
                          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-glocerova-red opacity-75"></span>
                          <span className="relative inline-flex rounded-full h-3 w-3 bg-glocerova-red"></span>
                        </span>
                    </>
                )}
            </button>
