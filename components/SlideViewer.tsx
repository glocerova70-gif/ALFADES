    ...
    return (
        <div className="relative w-full h-screen overflow-hidden bg-glocerova-dark">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-20 lg:px-32 max-w-7xl mx-auto">
                ...
                <div className={`transition-all duration-700 ease-out transform ${loaded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-glocerova-gold font-bold tracking-widest text-sm md:text-base uppercase font-sans">
                            {slide.subtitle}
                        </span>
                    </div>
                    ...
                    <p className="text-slate-300 text-lg md:text-xl max-w-2xl mb-10 leading-relaxed font-light">
                        {slide.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-4">
                        <button 
                            onClick={handleNext}
                            className="group flex items-center gap-4 bg-white text-glocerova-dark px-8 py-4 rounded-full font-bold hover:bg-glocerova-gold transition-colors duration-300"
                        >
                            {slide.ctaText}
                        ...
                        {slide.secondaryCta && (
                            <a 
                                ...
                                className="group flex items-center gap-3 border-2 border-white/30 text-white px-8 py-4 rounded-full font-bold hover:bg-white/10 hover:border-white transition-all duration-300 backdrop-blur-sm"
                            >
            ...
            <div className="absolute bottom-12 right-8 md:right-20 flex gap-4 z-20">
                ...
                <button 
                    onClick={handleNext}
                    className="p-4 rounded-full bg-white text-glocerova-dark hover:bg-glocerova-gold transition-all shadow-lg"
                >
            ...
            <div className="absolute bottom-12 left-8 md:left-20 flex gap-3 z-20">
                {sections.map((s, idx) => (
                    <button
                        ...
                        className={`h-1.5 transition-all duration-300 rounded-full ${s === currentSection ? 'w-12 bg-glocerova-gold' : 'w-6 bg-white/30 hover:bg-white/50'}`}
                    />
                ))}
            </div>
